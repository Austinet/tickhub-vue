...
//   const { dispatch } = useAuthContext();
  const newTicket = ref(props.ticket);
  const newTicketErrors = ref(ticketErrors);
  ...
  const updateTicket = () => {
    ...
    if (!TITLE_REGEX.test(newTicket.title)) {
    ...
    if (!DESCRIPTION_REGEX.test(newTicket.description)) {
   ...
   const closeForm = () => {
    props.toggleShowForm('UPDATE_TICKET')
    success.value = false
  }
</script>
